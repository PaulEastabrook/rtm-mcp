...
RECEIPT_CONTRACT: dict[str, Any] = {
    ...
    "advisory": (
        "Something about the CALL worth checking. Not a rejection and never blocking. THREE "
        "independent producers, and more than one can appear in the same string. (a) LOST "
        "VALUE — the call arrived carrying none of this tool's optional parameters, which are "
        "named. Two causes are on record and the server cannot tell them apart: the value was "
        "emitted as literal tool-call markup folded into another parameter (a stray `</…>` or "
        "`<parameter name=…>` tail — the key never existed and the text is written VERBATIM "
        "into the record), or some MCP clients silently DROP a misspelt optional before the "
        "server sees it. If you sent an optional and see this, it did not arrive — check your "
        "own text for markup first, then re-send with the exact name. (b) LEAKED MARKUP — that "
        "markup was actually FOUND in a string argument; it names the carrier and what did not "
        "arrive, and fires even when other optionals did. (c) NAME LENGTH (gtd_item_create / "
        "gtd_project_create) — the name exceeds 60 characters, which usually means an outcome "
        "statement, an acceptance criterion or a date is sitting in the title field. That one "
        "is GTD hygiene, not an error: the write landed in full."
    ),
    "how_to_use": (
