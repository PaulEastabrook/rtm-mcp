...
├── gtd_writes.py       # Phase 1 writes — pure grammar for the 4 everyday governed write tools: the SEVEN Tier-1 server-owned vocabularies (D1), structural tag materialisation, the hard-gated per-kind Definition of Ready, note title/block-order grammar, and the validators (backs gtd_create_item / gtd_add_note / gtd_capture / gtd_transition_state)
