...
| `project_index.py` | Pure (no IO) active-`#project` portfolio roll-up backing `gtd_project_index` — `build_index` (per-project rows: selection (incomplete, `#project`, not `#test`; `#hold`/`#someday` policy), life + parent Area-of-Focus resolution, and counts `open_count`/`blocked_count`/`next_tickle` via `project_plan.build_envelope` + the thin `plan_graph.build_graph`), `build_foci` (every `#focus` area incl. project-less ones), and `build_actions` (every incomplete child under an active project — with `due`/`priority`/`blocked` urgency signal for the What's-hot band — for cockpit search). Vault-free |
...
- `build_actions` — every incomplete child under an active project (actions + waiting-fors + calendar
  ...
  project_id, project, focus, life, due, priority, blocked}`, sorted `life → focus → project → name`.
  Reuses `project_plan.build_envelope` for each active project's rows, so attribution matches the
  canvas; every row carries a real project (no dangling-project rows; a top-level project's actions
  inherit `focus="(unfiled)"`). The three urgency fields (since v1.11.0, for the "What's hot" band)
  come from work already done: `due` is the row's localised own date (`""` when none), `priority`
  the `"1"|"2"|"3"|""` encoding shared with the project rows, and `blocked` the per-row judgement of
  the **same thin `plan_graph.build_graph`** that feeds each project's `blocked_count` (so they
  agree by construction — an open `DEPENDS-ON` upstream within the project's own rows).
