...
├── tools/
│   ├── tasks.py        # Task CRUD + metadata + hierarchy + occurrence listing (20 tools)
...
| `parsers.py` | Translate RTM's quirky API responses into clean Python dicts. Also home of the two recurrence-derived facts every other module reads rather than re-deriving: `repeat_kind` (the `rrule/@every` discriminator) and `entity_handle` (the durable GTD handle `entity_id` + `recurring`) |
...
| `models.py` | Schema-only Pydantic models generating each tool's MCP `outputSchema` (attached via `@mcp.tool(output_schema=...)`); **not used at runtime** — tools still return the `response_builder` dict, FastMCP advertises the schema without validating. `data` is always a `success \| ErrorData` union — since v2.0.0 `ErrorData.error` is the nested `ErrorBody` (`{code, message, rtm_code, details}`, `extra="forbid"`), not a prose string; the six-surface tool-documentation standard (CONTRIBUTING § 3). `_write_envelope_schema` mixes the `Receipt` in for a governed write and — since v6.7.0 — **raises** if the success model declares a `RECEIPT_FIELDS` name: the mixin sits BEHIND the result model, so a collision lets the tool win the advertised schema while `receipt.attach` wins at runtime, and both halves then work while disagreeing (`CreateItemResult.advisory`, v4.0.0–v6.6.0) |
...
| `companion.py` | The vault file-IO seam: locate the read-only AI Memory vault root (cross-platform; `AI_MEMORY_DIR`/host default + `memory/_index.md` marker), resolve each filed artefact's companion (`.md`/`.yaml`) frontmatter, and enrich `gtd_project_canvas` file objects with a `meta` block. `walk_artefacts` enumerates the vault ONCE for `gtd_note_filing_gaps`; since v6.5.1 its directory pruning is **derived** (`_is_artefact_dir`: any `.`- or `_`-prefixed directory, plus the named `node_modules` / `system`) rather than a hand-maintained list, because the hand-maintained one never learned about `.stversions` and so enumerated Syncthing's versioned COPY of a real artefact — companion and all, so it passed the tracked-ness filter and read as an orphan that could never be journalled. Live effect: 2,731 → 1,273 files enumerated, 184 → 160 tracked, zero real artefacts lost. **v6.5.2 closed a second two-lists-must-agree gap in the same walk**: `companion_candidates` knows FIVE companion forms and `_COMPANION_SUFFIXES` only THREE, and the missing form 2 (`X.md` is the companion of a non-md `X.<ext>`) **cannot be expressed as a suffix at all** — it depends on a SIBLING's existence. 49 companions were being enumerated as artefacts; `resolve_companion_path` now derives the claimed set from the same candidate list the resolver uses (1,273 → 1,224, tracked unchanged at 160). Mirrors file-store's `query_outputs.py` by contract (stdlib-only). Graceful: every IO failure → no `meta`, never raises |
...
| `filing_gaps.py` | The RTM↔vault **reconciliation** behind `gtd_note_filing_gaps`. This server is the only process that can see both sides (agent-memory-mcp holds no RTM token; RTM holds no vault), which is why the join is computed here rather than by an agent eyeing two tool outputs. Seven classes — `linked_missing` / `filed_unlinked` / `companion_missing` / `join_unpopulated` / `prose_path` / `register_defect` — each with its measured 2026-08-01 baseline in the docstring. `prose_path` **detects, never parses**: ten mutually incompatible dialects were counted, so the note is reported for a human. `legacy_unfiled` (v6.5.0) is the pre-v6.4.0 `FILING: <path> (unfiled)` form — a declaration of absence, so neither a broken link nor prose; RTM-only, so it keeps answering with no vault. **An absent vault produces a PARTIAL result** — `VAULT_DEPENDENT` classes are named in `gaps[]` and emitted as `null`, never `0` (the `gtd_engine_report` precedent); `walk_artefacts` returning `None` vs `[]` is what carries that distinction |
...
### Name length as a GTD hygiene signal (v6.6.0) — and the membrane it must not cross

Implements the approved designed change `2026-08-02-vault-naming-mirrors-rtm` §§ 1a.1, 1a.3,
Moves 7 and 8. `gtd_item_create` and `gtd_project_create` emit a receipt `advisory` when the raw
name exceeds **60 characters** (Paul, 2026-08-02).

**The value is not "your folder will be truncated".** Vault folder names are now derived from the
RTM item name, but the measured payoff is elsewhere. Live estate, 2026-08-02: live project names
run **45% longer** than archived ones (median slug 46.5 vs 32.0); of the 31 live projects over
budget, 18 come under by cutting at the first `—` or `(`, and **13 are defects the length merely
exposed** — nine outcome statements sitting in the title field ("…(ensure this is objective and
data backed)" is an acceptance criterion), four single actions mis-tagged as projects, two
carrying a date that belongs on the due date. So the claim is *"something is in the wrong field"*,
which is a judgement the caller must make — hence an advisory that can never become a gate.

**⚠ THE MEMBRANE — the constraint most likely to be eroded by a later change.** This server owns
**one integer and a comparison**. It does not import the vault naming rule, does not know what a
folder is, and its message speaks of name length only — no path, no slug, no folder. An earlier
draft of the designed change gave this repo the slug function, the path template and the length
budget; that was corrected *before* implementation (§ 1a.1) precisely because it breached the
boundary `companion.py` marks (the ONE vault seam, read-only, documented as never to widen). The
filesystem reasoning lives in gtd's `references/focus-area-map.md`. Two tests fail loudly on a
re-breach: an `ast` sweep for an import of the naming rule, and a scan for a vault path template
in any module but `companion.py` — both watched to fail under a deliberate breach rather than
merely observed green.

**⚠ It is a deliberate ONE-SIDED proxy, and lowering the number does not make it sound.** The
advisory measures the RAW NAME; truncation is decided by the SLUG, computed vault-side. Measured:
**6** live items truncate without tripping a 60-character advisory (a 56-character name losing its
last word); **zero** trip it without truncating. The error is strictly one-sided — under-warns,
never over-warns, which is the safe direction. And a raw-name threshold is unsound *in principle*
rather than merely imprecise: slugging **expands** as well as contracts (`&` → `and`), so
`R&D & QA & Ops & Sec review` is 27 characters and slugs to 37. Reaching for a sounder threshold
here would mean importing the slug rule. The band is closed at the other end instead —
`agent-memory-mcp` reports ACTUAL truncation at filing time, where `folder_name()` can compute it.
Both docstrings state the caveat.

**Appended, not ranked.** `attach` concatenates the name advisory onto whichever loss advisory
fired (markup, else bare-call). Markup and bare-call are mutually exclusive because one *explains*
the other; name length explains neither and is explained by neither — it is an observation about
data that DID land, where the other two are about data that may not have. Ranking it would
silently drop a true signal.

**Silent by construction, not by exemption.** `tools/gtd.py::_tool` accepts `name_of`, an
extractor supplied at the two registration sites (`name`; `frame["name"]`, coerced because
`frame` may arrive as a JSON string). Every other tool passes nothing, so `item_name` is None and
the producer cannot fire. A callable rather than a dotted-path string — two small lambdas beat a
path mini-language nothing else would use.

**Membrane / activation.** Vault-free, **no new tag**, no new `ErrorCode` (a new member
re-fingerprints all 102 tools for a non-failure the advisory already states), no `not_applied[]`
entry (nothing was withheld), no gate. **Two fingerprints churn**, description-only. To go live:
restart the server on v6.6.0. Rollback is a revert.

**Found in passing, and FIXED in v6.7.0 — see the next section.** `gtd_item_create` set
`data["advisory"] = val["advisory"]` (the Definition-of-Ready `relational` axis, a `list[str]`)
and `receipt.attach` then overwrote that key unconditionally, so the DoR advisory reached a caller
zero times from v4.0.0 to v6.6.0. The axes are `advisory_axes` from v6.7.0 and the collision class
is refused at import.

### The receipt owns three field names — `advisory_axes` and the collision guard (v6.7.0)

`gtd_item_create` and the receipt both claimed `advisory`, and **both halves worked while
disagreeing with each other**: `models._write_envelope_schema` mixes `Receipt` in BEHIND the
result model, so `CreateItemResult.advisory: list[str]` won the MRO and the tool advertised
`array of string`, while `receipt.attach` — which assigns unconditionally — won at runtime and
wrote `str | None`. **Neither half could fail a test.** The schema tests read the schema, the tool
tests read the runtime, and nothing compared the two. That is the whole shape of the defect, and
it is why it survived three releases behind a green suite.

**The DoR axes moved to their own key rather than merging into the receipt's prose**, on three
grounds, the second decisive: the receipt's `advisory` must stay one type across all 25 governed
writes (the collision IS the defect); `ADVISORY_AXES` is a **constant lookup by kind**
(`("relational",)` for an action, `()` otherwise), so merging would append a fixed sentence to
100% of action creates — the exact noise rule `receipt.is_facet` already exists to enforce, having
been measured at two tools firing on 100% of legitimate calls; and `advisory_axes` reads as
`missing`'s sibling, same `list[str]`, opposite gate, so a consumer iterates a list instead of
searching a substring. It is not a fourth advisory producer and does not interact with v6.6.0's
append-not-rank rule — the axes were never about the *call*, only about the *item*.

**The durable half is the guard, not the rename.** `_write_envelope_schema` now **raises
`TypeError`** when a success model declares any `receipt.RECEIPT_FIELDS` name — the v6.0.0 posture
of making a defect unrepresentable rather than merely rejected. There is no legitimate collision
(the three fields are the receipt's by definition), the failure is silent by construction, and
raising at import means it cannot reach a caller. Beside it sits the assertion that was missing
all along: every governed write's advertised `data.advisory` is `string | null`, checked over the
real server — the direct comparison of the two surfaces.

**Membrane / activation.** Vault-free, **no new tag**, no new `ErrorCode`, no gate. **One
fingerprint churns.** The runtime value of `data["advisory"]` is unchanged (it has been the
receipt's string since v4.0.0 and the advertised list has never once been sent), so no consumer
can have been reading the axes from it — hence minor, not major. To go live: restart the server on
v6.7.0. Rollback is a revert.
...
**⚠ v6.5.0 — the legacy `(unfiled)` marker, and why it was a correctness bug rather than a
cosmetic one.** Pre-v6.4.0 notes declare "nothing was filed" as `FILING: <path> (unfiled)` on a
real FILING line, because `unfiled=True` did not exist yet. Two consequences, and only the first
was reported:

1. `gtd_note_filing_gaps` counted them as `linked_missing` — *"a path that resolves to nothing"*.
   Wrong on the merits: nothing is missing, nothing was ever filed. Now its own class,
   `legacy_unfiled` — a migration backlog with a natural end state. **Not** folded into
   `prose_path`, which means "no FILING line at all" and would be a description that does not fit;
   filing a finding under the wrong name is how vocabularies rot.
2. **The one that mattered.** `_clean_filing_path` strips only the companion marker, so
   `work/…/x.md (unfiled)` is non-empty, relative and backslash-free — it passed every check and
   `parse_filings` returned it as a real path. Since `parse_filings` backs `gtd_chat_thread`, the
   board rendered **an attachment for an artefact that was never filed**. That is precisely the
   failure v6.4.0's `UNFILED:`-on-its-own-line design prevents; the reasoning was right and simply
   was not applied backwards to the notes already in RTM.

`is_legacy_unfiled` is a **predicate, not a change to `_clean_filing_path`'s return**, because the
two consumers want opposite things: `parse_filings` skips it (no phantom attachment) while the
audit keeps it (silently dropping a legacy form makes a backlog invisible). `parse_filings` and
`legacy_unfiled_paths` are two typed views over ONE line-walker (`_filing_payloads`), so the
two-line continuation form cannot work in one and not the other. Live population: 3, all eval
fixtures — which is why the cosmetic half was genuinely low-noise and the correctness half was not.

**⚠ v6.5.1 — the last two noise sources, both found on the run after v6.5.0.** Neither is a
regression; both were masked by the `filed_unlinked` noise v6.4.1 removed, which is the argument
for re-running the report after every fix to it rather than assuming the last fix was the last
defect.

**A — `.stversions/` ghosts counted as filed artefacts.** `_SKIP_DIRS` was hand-maintained and
never learned about Syncthing's version history, so a versioned COPY of a real artefact was
enumerated — and because the whole folder is versioned it carried a companion, passing v6.4.1's
tracked-ness filter and reading as an orphan that can never be journalled. Same root cause as
three other defects this programme has fixed. The rule is now **derived**
(`companion._is_artefact_dir`), which also resolves an inconsistency: `_`-prefixed *files* were
skipped while `_`-prefixed *directories* were walked.

**B — a whole sentence accepted as a filing path.** An **eleventh** prose dialect, and the one
the model did not cover: the ten counted at the census sit *instead of* a FILING line (which
`prose_path` reports); this one sits **on** one, so after the companion marker is stripped it is
non-empty, relative and backslash-free and passed every check. The reconciliation reported the
artefact MISSING — it is not missing; the parser could not say *"that isn't a path"*.

`gtd_chat.is_bare_path` tests for a **trailing file extension**, and emphatically NOT for
whitespace: the live estate holds legitimately-spaced filenames (`Job Spec - Delivery Leader -
24-Mar-2026.pdf`), and a rule that rejects real paths is worse than the defect it fixes. One rule
covers every observed malformation — a trailing clause ends in prose, an unsanctioned
parenthetical ends in `)`, a sentence ends in `.` with nothing after it.

Three consumers, one predicate: `_clean_filing_path` returns None (skipped, never repaired);
**`gtd_note_report.filing_path` reports it** — that class existed with the boundary drawn
correctly and reported `0` live, simply because the check was not strict enough to fire; and
`gtd_note_filing_gaps` needed no change, the row dropping out of `linked_missing` and landing in
`prose_path`, which is now the honest classification (the note genuinely carries no
machine-readable FILING line). `note_report._filing_findings` consumes `gtd_chat._filing_payloads`
rather than walking the lines itself — a third parser is what this programme keeps refusing to
write.
...
### Recurrence: two kinds, one attribute, and `taskseries_id` means different things (v6.8.0)

RTM has **two** recurrence kinds and they differ in the one property downstream code most wants to
rely on — whether an identifier survives:

| Kind | Structure | Identity across occurrences |
|---|---|---|
| `every` ("every 2 weeks") | ONE taskseries, MANY tasks; name/tags/notes shared | `taskseries_id` **stable** |
| `after` ("after 2 weeks") | a **NEW taskseries per occurrence**, properties copied | **nothing survives** — both ids re-key |

**RTM sends `<rrule every="0|1">`, and that attribute IS the discriminator** (`1` = every-type,
`0` = after-type). Through v6.7.0 `parsers.py` did `is_repeating = bool(ts.get("rrule"))`, so the
whole element collapsed to a boolean one line into the parser and the kind was **discarded, not
unobtainable**. It is checked-three-ways unobtainable *anywhere else*: MilkScript exposes only a
bare `isRecurring()`, `rtm.Recurrence` is a **write-only builder** with no `getRecurrence` on
`TaskSeries` or `Task`, and the search syntax has `isRepeating:true` but **no repeat-type
operator**. This parser line is the only place the fact exists.

`parsers.repeat_kind(rrule)` is the whole rule. Measured shape (live, 2026-08-03): the attribute
arrives as the **string** `"1"`/`"0"` on a dict that also carries the rule text in `$t` — e.g.
`{"every": "1", "$t": "FREQ=WEEKLY;INTERVAL=1;WKST=MO"}`.

**Three-valued, and `None` is overloaded on purpose.** `"every"` | `"after"` | `None`, where
`None` means *not repeating* **or** *a rule that could not be classified* — and `is_repeating`
beside it is what separates the two. **Read them as a pair.** A consumer that treats a bare
`repeat_kind is None` as "not repeating" is wrong on the fourth case.

**⚠ Never default an unreadable rule to `"every"`.** That is the silent-wrong-identity failure the
field exists to prevent: a consumer keying durable state on `taskseries_id` would key it on an id
that re-keys, and nothing would say so. Unreadable must read as unreadable — a test pins it.

**⚠ RETRACTED at v6.9.0 — the "free, sound cross-check" this section shipped is FALSE.** It read:
*a taskseries holding ≥2 tasks is provably `every`, since an `after` recurrence cannot produce two
tasks in one series … verified over the whole account: of 31 such series, all 31 classified
`every`.*

Two things were wrong, and the second explains the first. **It was not verified over the whole
account** — the census ran over `status:incomplete`, 1,162 of the account's 44,730 tasks. Over all
of them, **11 `after` series hold ≥2 tasks**, the largest **86** deep (`226592019` "Taken protein
shake?", every task carrying `every="0"`). An `after` series accumulates its completed occurrences
exactly as an `every` one does; what re-keys is the series a NEW occurrence lands in, not the
history already recorded. The subset held only because an `after` series has at most ONE **open**
occurrence — the next is minted on completion — which is precisely what made the subset
unrepresentative of the property being checked.

**Counting is unsound in the other direction too**: **325** series that do not repeat at all hold
≥2 tasks (one 31 deep — `147643653`), because deleting a recurrence rule leaves its occurrences
behind. So neither `is_repeating` nor the kind can be inferred from a count. **`rrule/@every` is
the only discriminator.**

Nothing executable depended on the claim — it was offered to callers in prose — but the test that
"covered" it asserted `repeat_kind({"every": "1"}) == "every"`, which could not have failed
whatever the live data said. That is how it shipped behind a green suite. It is now pinned by the
real counter-example instead.

**Where it travels:** the parsed task; `project-plan-seed/3.1` (`header.project` + every row);
`gtd_item_stamp_tokens`; `list_tasks`, which gained **both** fields since it carried neither
(`repeat_kind` alone is undecodable there); and, since v6.9.0, `list_task_occurrences`.
`gtd_project_index` carries neither `is_repeating` nor `taskseries_id`, so there is nothing for a
kind to qualify — deliberately untouched (it does carry `entity_id`, which is a different
proposition; see the next section).

Live population (2026-08-03): 91 `every` series, 27 `after`, **zero** unclassified.

### The durable entity handle — `entity_id` + `recurring` (v6.9.0)

The consumer should not have to learn how RTM works. The vault stores **one id and one boolean**;
it never sees a `taskseries_id`, never learns there are two recurrence kinds, and holds no
current-occurrence id — so something must resolve the handle, and the `gtd_*` layer is the
canonical RTM wrapper.

**The mixup is measured, not theoretical.** On 2026-08-02, six of 37 rename ids were
`taskseries_id`s passed as `task_id`s; they matched no task and the operation survived only
because it fell back to matching on name. Two id fields differing by an RTM internal keep
producing that; one field with one meaning cannot.
...
**`entity_id` is NEVER absent**, so a consumer never has to interpret an absence. **An `after`
occurrence is not recurring** (Paul, 2026-08-03): RTM mints a new taskseries per occurrence sharing
only a name — its own docs say such a task *"isn't tied to previous tasks"* — so it is an
independent one-off and its `task_id` is a perfectly good durable handle. That reclassification is
what removed the refusal case an earlier draft carried.

**`recurring` means ONE COMMITMENT WITH MANY INSTANCES — nothing weaker**, so it is `true` only for
`every`. It is never re-derived from the presence of an id (every task has a `taskseries_id`) nor
from a count of occurrences (unsound in both directions — see the retraction above).

**One guard, and its direction is deliberate.** An `every` classification with no usable
`taskseries_id` degrades to `(task_id, False)`. Structurally unreachable — the `rrule` hangs on the
very taskseries whose id it is — but `recurring=False` on a truly recurring item costs a folder per
occurrence (ugly, recoverable), whereas `recurring=True` on a re-keying id is the
silent-wrong-identity failure the field exists to prevent. The invariant it buys is checkable and
tested: **`recurring` is true iff `entity_id` is the taskseries id.**

**Derived once** in `parsers.entity_handle` and read from there by every surface, so no two reads
can disagree. `project_index.build_actions` takes the handle straight off the envelope row rather
than re-deriving it, which makes cockpit/plan agreement hold by construction — a mutation that
re-derived it survived the first mutation pass only because the fixture had no `every`-repeating
*action*, and the fixture was fixed rather than the assertion weakened.

**Where it lands, and why those four.** The identity-bearing `gtd_*` reads — `gtd_project_plan`
(header + every row, the envelope the other two draw from), `gtd_project_canvas` (frame + every
seed item), `gtd_project_index` (project, focus **and** action rows), `gtd_item_context` (the
single-item bundle, and the read where the two-id choice was most exposed). `format_task` carries
it too, so the transition off the generic tier is free. Detector / candidate reads are deliberately
untouched: they return work-lists whose consumer goes to the plan or to `gtd_item_context` for
identity.

`gtd_project_index` was a judgement, not a mechanical extension: it deliberately carries neither
`is_repeating` nor `taskseries_id`, but `entity_id` is a *domain handle* rather than an RTM
internal, and the cockpit is exactly where a consumer resolves a project to its vault folder.

**`taskseries_id` is RETAINED wherever it already appeared.** Removing it is breaking, and the gtd
`series_guard` genuinely needs it. Its removal from the domain reads is sequenced with the
generic-tier exclusion Paul will signal; until then, `entity_id` is what a domain consumer should
read and the raw ids are the escape hatch.

### `list_task_occurrences` — the occurrence door, deliberately generic-tier (v6.9.0)

`list_task_occurrences(taskseries_id, list_id=None, include_completed=True)` returns every task
instance RTM holds under one series, oldest-first, with `current` marking the open ones (deleted
excluded).

**It is `rtm_*` and not `gtd_*` by decision** (Paul, 2026-08-03). Enumerating occurrences is RTM
plumbing; its real consumers are tooling — a migration, a backfill, a reconciliation — not a GTD
workflow. An earlier draft specced it as `gtd_item_occurrences` and justified it as a weekly-review
read, which was a tool invented first and a use case reasoned backwards to fit it. **The domain set
gains nothing in this piece, and that is the correct outcome**: a wrapper should carry only what
the domain uses.

**RTM's search syntax has no taskseries operator**, so the series is matched client-side off ONE
`getList` — which is what makes `list_id` a real cost knob rather than an ergonomic one. Measured
2026-08-03: `status:incomplete OR status:completed` returns **44,730** tasks account-wide and
**15,596** scoped to one list, against **1,162** for `status:incomplete` alone.

**`current` is NOT singular.** An `every` series can hold several open occurrences at once —
`File company accounts` (476408903) has two, due 2026-09-01 and 2027-04-07; `Weekly GTD review`
(237677328) has two of its 17. A test asserting one current row would pass on most repeating
projects and be wrong.

**A miss reuses `TASK_NOT_FOUND` rather than minting a code** — the documented ladder: a new
`ErrorCode` member re-fingerprints all 103 tools, and a series miss *is* a task-resolution miss.
...
- `tests/test_response_builder.py` — envelope builder, transaction info, record_and_build_response, parsers (incl. `is_repeating` from the taskseries `rrule`); and the v6.8.0 **recurrence kind** — `TestRepeatKind` over the classifier (both live shapes verbatim from the account census; **the fourth case asserted as `None` and NOT defaulted to "every"**, which is the silent-wrong-identity failure the field exists to prevent; a non-string `every` still classifying, so the rule does not hinge on the wire type; and, at v6.9.0, the **RETRACTION** of the deductive ≥2-tasks cross-check — pinned by the live counter-example (`every="0"` on the 86-deep series 226592019) rather than by the previous test, which asserted `repeat_kind({"every": "1"}) == "every"` and so could not have failed whatever the data said; plus the other direction, a 31-deep series carrying no rule at all), the kind travelling out of `parse_tasks_response` beside `is_repeating`, and `format_task` carrying **both** fields — asserted as a PAIR incl. the `is_repeating=True, repeat_kind=None` row, since collapsing that into a plain one-off is the whole failure mode; and v6.9.0 `TestEntityHandle` — the durable handle per kind, `entity_id` NEVER absent across every kind/series combination, `recurring` a real bool never a truthy string, the **invariant `recurring` is true iff `entity_id` is the taskseries id**, the deliberate degradation direction when an "every" carries no series id, and that it is never re-derived from the presence of an id; `format_task` carrying the handle beside the retained raw ids (66 tests)
...
- `tests/test_tools/test_task_tools.py` — all 20 task tools via FakeMCP, incl. strict-tag-mode gating, unknown-list_name error paths (add_task/list_tasks), user-filter parenthesization, day-scale estimates; the list-target gate (add_task accepts writable / rejects smart + locked without writing; move_task same; default-list fallback NOT gated; flag-off inert) + `TestTheShippedDefaultIsLiveEndToEnd` (v5.1.0 — the same paths over a REAL `RTMConfig`, proving shipped default -> config -> tool wiring rather than a forced flag; the default-list carve-out re-asserted against the live default, where a locked configured default would otherwise reject every bare capture on activation); v6.9.0 `TestListTaskOccurrences` — `current` is NOT singular (the live 476408903 shape, two open at once); oldest-first with undated LAST and a task_id tie-break; an "every" series reporting the series as its entity; **an `after` series holding several occurrences still classified `after`** (the live 226592019 shape — the counter-example that retracted v6.8.0's deductive rule) and **a non-repeating series holding several** (325 live ones do, because deleting a repeat rule leaves its history); other series excluded (the match is client-side, RTM has no taskseries operator); deleted occurrences excluded; a miss typed `task_not_found` and naming the fix, with the include_completed=False variant naming the retry; the read-only call surface with no transaction; and BOTH cost knobs asserted as reaching the wire (102 tests)
