...
**A free, sound cross-check.** A taskseries holding **≥2 tasks is provably `every`**, deductively,
since an `after` recurrence cannot produce two tasks in one series. It costs nothing from the same
read and can never wrongly bless an `after` series; it cannot confirm a never-yet-recurred series,
which stays unknown. Verified over the whole account: of 31 such series, all 31 classified
`every`, zero violations.

**Where it travels:** the parsed task; `project-plan-seed/3.1` (`header.project` + every row);
`gtd_item_stamp_tokens`; and `list_tasks`, which gained **both** fields since it carried neither
(`repeat_kind` alone is undecodable there). `gtd_project_index` carries neither `is_repeating` nor
`taskseries_id`, so there is nothing for a kind to qualify — deliberately untouched.
