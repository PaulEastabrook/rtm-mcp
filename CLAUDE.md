...
**⚠ Found in passing, NOT fixed, and coupled to any future change here.** `gtd_item_create` sets
`data["advisory"] = val["advisory"]` — the Definition-of-Ready `relational` axis, a `list[str]`
its docstring promises is "REPORTED in `advisory`" — and `receipt.attach` then **overwrites that
key unconditionally**, so the DoR advisory has never reached a caller. The advertised schema
disagrees with the runtime too: `models.CreateItemResult.advisory` advertises `array of string`
while a string is written. Pre-existing, out of this brief's scope, and a fix changes an advertised
output schema — it must decide whether the DoR axes move to their own key or the two advisories
merge.
