...
| `project_plan.py` | Pure (no IO) reconstruction of the `project-plan-seed/3` envelope from parsed tasks — byte-compatible with the gtd plugin's `rtm_fetch.py` reference. Also the home of the `REDACTED_TAG` constant and emits the additive `header.project.redacted` flag + the additive per-note `id` (every envelope note object carries the RTM note id — the ORDER-note resolver tie-breaks on it) |
