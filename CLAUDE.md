...
├── gtd_chat.py         # Pure CHAT-note grammar (title/mode-footer/turn/thread parsing) + turn attachments (FILING/LINK parse + correlation) + drain-signal tags + cross-project inflight roll-up (backs gtd_chat_post/gtd_chat_thread/gtd_chat_inflight)
...
**Turn attachments (`files[]` / `links[]`, since v1.19.0 — board-chat enrichment stage 2).** Every
...
Same-task only in v1 — the cross-child project-scope walk is a later increment. Correlation runs
